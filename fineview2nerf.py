...
def main():
    ...
    ap.add_argument('--allow_heuristic_bounds', action='store_true',
                    help='Permit the camera-distance near/far fallback when '
                         ...
                         'heuristic range is ~20x too wide, which starves '
                         'the object of samples and trains to fog.')
    args = ap.parse_args()
    ...
    if len(pts) == 0 and not args.allow_heuristic_bounds:
        raise SystemExit(
            'ERROR: no 3D points in sparse/0 (points3D.bin/.txt missing or '
            'empty) — near/far bounds would be a camera-distance heuristic '
